...
public interface CategoryRepository extends JpaRepository<Category, Long> {
    ...
    @Query(nativeQuery = true, value = "SELECT categories.name, SUM(products.amount) FROM categories INNER JOIN products " +
            "ON products.id_categories = categories.id GROUP BY categories.name")
    List<CategorySumProjection> search1();

    @Query("SELECT new com.devsuperior.uri2609.dto.CategorySumDTO(obj.category.name, SUM(obj.amount)) FROM Product obj " +
